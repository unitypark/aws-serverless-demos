...
function AdminForm() {
  ...
  async function handleSubmit(files: File[]) {
    ...
    let path = `${unixTimestampInSeconds}/${files[0].name}`.replace(/\s/g, "").toLowerCase();

    const postUploadsRes = await client.post(`/uploads`, { path: path })
  ...
  var redirectURL = `${window.location.origin}/downloader?key=${response?.accessKey}`;
  const text = () => {
