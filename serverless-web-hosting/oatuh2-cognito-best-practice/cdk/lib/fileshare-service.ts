...
export class FileShareService extends cdk.Stack {
  constructor(scope: Construct, id: string, props: FileShareServiceProps) {
     ...
     * DynamoDB
     */
    const ddbTable = new Table(this, props.prefix + '-ddb-table', {
     ...
     * Cognito
     */
    const cognito = new CognitoUserPool(this, props.prefix + '-cognito-userPool', {
      region: this.region,
     ...
     * HTTP API
    */
    const httpApi = new HttpApi(this, props.prefix + '-http-api', {
     ...
     * S3 Website bucket 
     */
    const fileShareServiceWebSiteBucket = new Bucket(this, props.prefix + '-fileshare-service-website-bucket', {
     ...
     * Cloudfront 
     */
    const cloudfrontOAI = new OriginAccessIdentity(this, props.prefix + '-oai', {
    ...
    const cookieSettings = JSON.stringify({
      accessToken: `Path=/api; Secure; HttpOnly; Max-Age=3000; Domain=${props.domain}; SameSite=Lax`,
      idToken: `Path=/; Secure; HttpOnly; Max-Age=3000; Domain=${props.domain}; SameSite=Lax`,
      refreshToken: `Path=/refreshauth; Secure; HttpOnly; Max-Age=28800; Domain=${props.domain}; SameSite=Lax`,
    ...
    new BucketDeployment(this, props.prefix + '-deploy-fileshare-service-website-asset', {
      sources: [
     ...
     * S3 Asset
     */
    const fileShareAssetBucket = new Bucket(this, props.prefix + '-asset-bucket', {
      encryption: BucketEncryption.S3_MANAGED,
      removalPolicy: cdk.RemovalPolicy.DESTROY,
      autoDeleteObjects: true,
      blockPublicAccess: BlockPublicAccess.BLOCK_ALL,
     ...
     * API Lambda Function
    */    
    const getConfigHandler = new GoLambdaFunction(this, props.prefix + '-get-config', {
      name: props.prefix + '-get-config',
      entry: LAMBDA_GET_CONFIG_LOCATION,
    });

    const postUploadsHandler = new GoLambdaFunction(this, props.prefix + '-post-uploads', {
    ...
    const postDownloadsHandler = new GoLambdaFunction(this, props.prefix + '-post-downloads', {
    ...
    const getDownloadHandler = new GoLambdaFunction(this, props.prefix + '-get-download', {
     ...
     * Authorizer
     */
    const authHandler = new GoLambdaFunction(this, props.prefix + '-api-authorizer', {
    ...
    const lambdaAuthorizer = new HttpLambdaAuthorizer(props.prefix + '-cookie-authorizer', authHandler.fn, {
    ...
    httpApi.addRoutes({
      ...
      integration: new HttpLambdaIntegration(props.prefix + '-get-config-integration', getConfigHandler.fn, {
